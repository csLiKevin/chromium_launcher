...
export function renderProgressBar(downloaded: number, total: number, width = 40): string {
  if (total === 0) {
  ...
  const bar = "=".repeat(filled) + arrow + " ".repeat(Math.max(0, width - filled - arrow.length));
